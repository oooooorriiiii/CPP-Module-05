...
Form::Form()
    : _name("no"),
      _signed(false),
      _signGrade(khighestGrade),
      _executeGrade(khighestGrade) {
...
//	std::cout << "Constructor is called in Form" << std::endl;
  if (signGrade > klowestGrade || executeGrade > klowestGrade) {
    throw GradeTooLowException();
  } else if (signGrade < khighestGrade || executeGrade < khighestGrade) {
    throw GradeTooHighException();
  }
}

Form::Form(const Form &form)
    : _signGrade(form.getSignGrade()), _executeGrade(form.getExecuteGrade()) {
